 ...
 */
(function () {
    materialAdmin.factory('NewTaskService',['$rootScope','httpLoad','$modal', '$timeout', '$stateParams',	function ($rootScope, httpLoad, $modal, $timeout, $stateParams) {
		...
		NewTaskService.prototype = {
			...
			InitContent:function(){
				...
				this.scene.background = '/img/topology/bg.jpg';
			...
			InitTab:function(){
				...
				data.forEach(function (item) {
					...
					html.push('<img class="node-img" src="/img/topology/'+item.Image+'" />');
			...
			AddNode:function(x, y, str, img, textPosition, nodeType, nodeData){
				...
				if (null != img) {
					node.setImage('/img/topology/' + img, false);
					node.Image = img;
				}
			...
			AddScriptNode:function(node){
				var modalInstance = $modal.open({
					templateUrl: '/views/application/newtaskcopy/scriptModal.html',
			...
			AddFileNode:function (node) {
				var modalInstance = $modal.open({
					templateUrl: '/views/application/newtaskcopy/fileModal.html',
			...
			AddBackupNode:function (node) {
				var modalInstance = $modal.open({
					templateUrl: '/views/application/newtaskcopy/backupModal.html',
			...
			EveSet:function(){
				...
				var delNode = function (flag,node) {
					var modalInstance = $modal.open({
						templateUrl: '/views/application/newtask/delModal.html',
    ...
    materialAdmin.controller('NewTaskCopyCtrl', ['$scope', '$rootScope', '$stateParams', '$state','NewTaskService', 'httpLoad',
		function ($scope, $rootScope, $stateParams, $state, NewTaskService, httpLoad) {
			$rootScope.moduleTitle = '运维中心 > 新建作业';//定义当前页
			$rootScope.link = '/css/task.css';//引入页面样式
			...
			(function(){
				switch($stateParams.flag/1){
					...
					case 10:
						...
						saveUrl = '/task/graph/create';
						$scope.isHideExecute = true;
						$scope.pageGo = 'application.tasklist';
						break;
					case 11:
    ...
    materialAdmin.controller('addScriptTaskModalCtrl', ['$scope', '$modalInstance', '$modal', '$stateParams', '$timeout', 'httpLoad',  'node',
		function ($scope, $modalInstance, $modal, $stateParams, $timeout, httpLoad, node) {
		if($stateParams.flag == 10 || $stateParams.flag == 11) $scope.hideTarget = true;
			...
			$scope.selectServer = function(){
				var modalInstance = $modal.open({
					templateUrl: '/views/application/newtask/selectServerModal.html',
    ...
    materialAdmin.controller('addFileTaskModalCtrl', ['$scope', '$modalInstance', '$modal', '$stateParams', '$timeout', 'httpLoad',  'node',
		function ($scope, $modalInstance, $modal, $stateParams, $timeout, httpLoad, node) {
			$scope.itemsByPage = 5;//目标服务器每页显示条数
			node = angular.fromJson(node);
			$scope.identModeData = CommonData.identMode;
			...
			$scope.addServerFile = function () {
				var modalInstance = $modal.open({
					templateUrl: '/views/application/newtask/selectServerFileModal.html',
			...
			$scope.selectServer = function(){
				var modalInstance = $modal.open({
					templateUrl: '/views/applicationn/newtask/selectServerModal.html',
					controller: 'selectServerModalCtrl',
					backdrop: 'static',
					size:'lg',
					resolve: {
    ...
    materialAdmin.controller('addBackupTaskModalCtrl', ['$scope', '$modalInstance', '$modal', '$stateParams', '$timeout', 'httpLoad',  'node', 'CommonData',
