...
(function(){
	"use strict";
    materialAdmin.service('TaskData', function() {
		var service = {
			//定义任务状态
			statusModeData:{
				1: '未执行',
    ...
    materialAdmin.controller('TaskListCtrl', ['$scope', 'httpLoad', '$rootScope','$modal','$timeout','TaskData','$state',
		function($scope, httpLoad, $rootScope, $modal, $timeout, TaskData, $state) {
			$rootScope.moduleTitle = '运维中心 > 常规任务';//定义当前页
			...
			$scope.remove = function (id){
				var modalInstance = $modal.open({
					templateUrl: 'views/basis/tasklist/delTaskModal.html',
			...
			$scope.createTimeTask = function (item) {
				$modal.open({
					templateUrl : 'views/basis/tasklist/addTimeTaskModal.html',
	...
	//新增定时ctrl
    materialAdmin.controller('addTimedGeneralModalCtrl',['$scope','$modalInstance','httpLoad','item',
