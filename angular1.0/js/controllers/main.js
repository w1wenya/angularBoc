materialAdmin
    ...
    .controller('materialadminCtrl', function($timeout, $state, $scope, growlService){
        //Welcome Message
        growlService.growl('欢迎回来!', 'inverse')
    ...
    .run([  '$rootScope', '$window', '$location', '$state',  'httpLoad',
        function($rootScope,   $window,   $location,   $state,    httpLoad) {
            //消息提示框设为默认方法
            $rootScope.pop = function(text,  type,icon,from,align) {
                $.growl({
                        icon: icon,
                        title: '',
                        message: text,
