app.controller('AlarmStrategyAddCtrl', ['$rootScope', '$scope', '$modal', 'httpLoad', '$state', '$stateParams', '$timeout', 'LANGUAGE', 'CommonData', function($rootScope, $scope, $modal, httpLoad, $state, $stateParams, $timeout, LANGUAGE, CommonData) {
	$rootScope.moduleTitle = '参数管理 > 新增告警策略';
	...
	//如果为编辑，进行赋值
	if($stateParams.id){
		$rootScope.moduleTitle = '参数管理 > 编辑告警策略';
	...
	//对阈值进行验证
	$scope.validateValue = function(item,value){
		if(!item[value]) return;
		switch (value){
			case 'remind':
				if((item.warn && item.warn <= item.remind)||(item.danger && item.danger <= item.remind)){
					item.remind = '';
					return $scope.pop('提醒阈值应小于警告阈值和严重阈值','error');
				};
				break;
			case 'warn':
				if((item.remind && item.warn <= item.remind)||(item.danger && item.warn >= item.remind)){
					item.warn = '';
					return $scope.pop('警告阈值应大于提醒阈值且小于严重阈值','error');
				};
				break;
			case 'danger':
				if((item.remind && item.danger <= item.remind)||(item.warn && item.warn >= item.danger)){
					item.danger = '';
					return $scope.pop('严重阈值应大于提醒阈值和警告阈值','error');
				};
				break;
		}
