...
(function(){
	...
	app.controller('AlarmListCtrl', ['$scope', 'httpLoad', '$rootScope','$modal','$timeout','$state','LANGUAGE',
		function($scope, httpLoad, $rootScope, $modal, $timeout, $state, LANGUAGE) {
			$rootScope.moduleTitle = '参数管理 > 告警策略';//定义当前页
			...
			$scope.create = function(id){
				$state.go('app.config.addalarm',{id:id});
			}
			//删除告警策略
			...
			$scope.goDetail = function(flag,id){
				switch (flag){
					case 1:
						$state.go('app.config.alarmdetail',{id:id});
						break;
					case 2:
						$state.go('app.config.alarmupload',{id:id});
						break;
					case 3:
						$state.go('app.config.aurecord',{id:id});
						break;
				}
			}
		}
