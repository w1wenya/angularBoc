/**
 * Created by Zhang Haijun on 2016/11/2.
 */
(function(){
	"use strict";
	app.controller('AlarmStrategyRecordCtrl', ['$scope', 'httpLoad', '$rootScope','$modal','$timeout','$state','$stateParams','LANGUAGE',
		function($scope, httpLoad, $rootScope, $modal, $timeout, $state, $stateParams, LANGUAGE) {
			$rootScope.moduleTitle = '参数管理 > 策略下发记录';//定义当前页
			$scope.param = {
				page: 1,
				rows: 10,
			...
			$scope.getList = function(page){
				...
				httpLoad.loadData({
					url:'/alarm/strategy/record/list',
					method:'POST',
					noParam: true,
					data:$scope.param,
			...
			$scope.remove = function(id){
				...
				modalInstance.result.then(function() {
					httpLoad.loadData({
						url: '/alarm/strategy/record/remove',
						data: {id:id},
						success: function(data){
							if(data.success){
								$scope.pop('下发策略记录删除成功');
								$scope.getList();
							}
						}
					});
			...
			$scope.goDetail = function(id){
				$state.go('app.config.aurecorddetail',{id:id})
			};
