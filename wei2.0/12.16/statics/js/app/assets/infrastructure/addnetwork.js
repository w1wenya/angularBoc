app.controller('AddNetworkModalCtrl', ['$rootScope','$scope', '$stateParams', '$state', 'httpLoad','CommonData',
	function ($rootScope,$scope, $stateParams, $state, httpLoad,CommonData) {
		...
		(function () {
			switch ($stateParams.flag/1){
				case 1:
					$rootScope.moduleTitle = '基础设施 > 添加网络设备';
					$scope.modalName = '添加网络设备';
					break;
				case 2:
					$rootScope.moduleTitle = '基础设施 > 编辑网络设备';
					$scope.modalName = '编辑网络设备';
					initDetailEdit($stateParams.id);
					break;
				case 3:
					$rootScope.moduleTitle = '基础设施 > 网络设备详情';
					$scope.modalName = '网络设备详情';
					$scope.isDetail = true;
					initDetailEdit($stateParams.id)
					break;
				case 4:
		...
		$scope.save = function () {
			...
			httpLoad.loadData({
				...
				success: function (data) {
					if (data.success) {
						$scope.pop($stateParams.id ? '网络设备编辑成功' : '网络设备添加成功');
						$scope.goBack();
					}
				}
			});
