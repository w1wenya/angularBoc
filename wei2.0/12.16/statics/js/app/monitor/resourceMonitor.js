angular.module('app').controller('resMonitorModalCtrl', ['$rootScope', '$scope','$interval','$state','httpLoad','$stateParams',function($rootScope, $scope,$interval,$state,httpLoad,$stateParams) {
    $rootScope.link = '/statics/css/monitor.css';
    $rootScope.moduleTitle = '监控管理 > 资源监控';
    $scope.param = {
        rows: 10
    };

    var resMonitorData = JSON.parse(sessionStorage.resMonitorData);
    ...
    var interval = resMonitorData.timeInterval + 's';
    $scope.queryItem = {
        "host":resMonitorData.ip,
        "measurementType":resMonitorData.measurement,
        "type":"",
        "category":"",
        "aggregation":"",
        "beginTime":"" ,
    ...
    $scope.move = function(choose){
        ...
        if(choose){
            ...
            if($scope.action==1) $scope.queryItem.interval = "";
    ...
    $scope.goBack = function(){
        $state.go('app.monitor.room');
        var resLocation = JSON.parse(sessionStorage.getItem('resLocation'));
        var resLocation1 = {
            page: resLocation.page,
            location:'detail',
            node:resLocation.node
        };
        sessionStorage.resLocation = JSON.stringify(resLocation1);
    };
}]);