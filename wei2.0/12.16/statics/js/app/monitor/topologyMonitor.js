angular.module('app').controller('topoMonitorModalCtrl', ['$rootScope', '$scope','$interval','$state','httpLoad','$stateParams',function($rootScope, $scope,$interval,$state,httpLoad,$stateParams) {
    $rootScope.link = '/statics/css/monitor.css';
    $rootScope.moduleTitle = '监控管理 > 数据纵览';
    $scope.param = {
        rows: 10
    };

    var topoMonitorData = JSON.parse(sessionStorage.topoMonitorData);
    ...
    $scope.queryItem = {
        "ip":topoMonitorData.ip,
        "type":topoMonitorData.type,
        "category":topoMonitorData.category,
        "aggregation":"",
        "beginTime":"",
        "endTime":"",
        "interval":""
    ...
    $scope.goBack = function(){
        $state.go('app.monitor.datacenter');
        var topoLocation = JSON.parse(sessionStorage.getItem('topoLocation'));
        var topoLocation1 = {
            page: topoLocation.page,
            location:'detail',
            node:topoLocation.node
        };
        sessionStorage.topoLocation = JSON.stringify(topoLocation1);
    };
}]);