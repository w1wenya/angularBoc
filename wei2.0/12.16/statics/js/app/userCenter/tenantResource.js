    ...
    app.controller('bindtenantModalCtrl', ['$scope','httpLoad','$modalInstance','$stateParams',
        function($scope, httpLoad, $modalInstance, $stateParams) {
            ...
            $scope.selectAll = function(){
                for(var a in $scope.listData){
                    ...
                    if($scope.isSelectAll==true){
                        if(!item.expiredTime1) {
                            $scope.pop('请选择过期时间','error');
                            return false;
                        }
                    }
            ...
            var timeformat = function(time){
                if(time.length==0) return 0+time;
            };
            $scope.changeTime = function(expiredTime1,$index){
                angular.forEach($scope.listData, function(data,index){
                    if(index==$index) {
                        data.expiredTime = expiredTime1.format('YYYY-MM-DD HH:mm:ss');
                    }
                });
            };
            $scope.ok = function () {
                ...
                selectList.forEach(function (item) {
                    resources.push({resId:item.id,cpu:item.cpu,memory:item.memory,disk:item.disk,expiredTime:item.expiredTime})
                });
                for(var i=0;i<selectList.length;i++){
                    var item = selectList[i];
                    if(!item.expiredTime||item.expiredTime=="") {
                        $scope.pop('绑定的每项资源都需要选择过期时间','error');
                        return false;
                    }
                }
                httpLoad.loadData({
