    ...
    .run([
      ...
      function ($templateCache) {
        $templateCache.put('template/treeGrid/treeGrid.html',
          "<div class=\"table-responsive maginleft\">\n" +
          ...
          "       <th ng-if='actionData'>操作</th>\n" +
          "     </tr>\n" +
          "   </thead>\n" +
          "   <tbody>\n" +
          "     <tr ng-repeat=\"row in treeRows | searchFor:$parent.filterString:expandingProperty:colDefinitions track by row.branch.uid\"\n" +
          "       ng-class=\"'level-' + {{ row.level }} + (row.branch.selected ? ' active':'')\" class=\"tree-grid-row\">\n" +
          "       <td><a ng-click=\"user_clicks_branch(row.branch)\"><i ng-class=\"row.tree_icon\"\n" +
          "              ng-click=\"row.branch.expanded = !row.branch.expanded\"\n" +
          "              class=\"indented tree-icon\"></i>\n" +
          "           </a><span class=\"indented tree-label tree-title-limit\" ng-click=\"on_user_click(row.branch)\">\n" +
          "             {{row.branch[expandingProperty.field] || row.branch[expandingProperty]}}</span>\n" +
          "       </td>\n" +
          "       <td ng-repeat=\"col in colDefinitions\">\n" +
          "         <div ng-if=\"col.cellTemplate\" compile=\"col.cellTemplate\" cell-template-scope=\"col.cellTemplateScope\"></div>\n" +
          "         <div ng-if=\"!col.cellTemplate\"  class=\"tree-text-limit\">{{row.branch[col.field]}}</div>\n" +
          "       </td>\n" +
          "       <td ng-if='actionData' style='width: 240px'>" +
          "       <button ng-repeat='item in actionData' ng-hide='item.flag == 1&&row.branch.type == \"beans\"' ng-click='user_clicks_action({flag:item.flag,item:row.branch})' class='btn m-r-xs' ng-class=\"{1:'btn-primary',2:'btn-info',3:'btn-danger'}[item.flag]\"><i class='fa'  ng-class=\"{1:'fa-plus',2:'fa-pencil',3:'fa-trash-o'}[item.flag]\"></i> {{item.name}}</button>"+
          "     </tr>\n" +
          "   </tbody>\n" +
          " </table>\n" +
          "</div>\n" +
      ...
      function ($timeout,
        ...
        return {
          ...
          link       : function (scope, element, attrs) {
            ...
            scope.user_clicks_branch = function (branch) {             //点击展开
              if (branch !== selected_branch) {
                return select_branch(branch);
              }
            };
            scope.user_clicks_action = function (data) {
              return $timeout(function () {
                return scope.treeAction({
                  data:data
                });
              });
            };
