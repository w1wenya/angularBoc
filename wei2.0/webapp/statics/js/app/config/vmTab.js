(function(){
    app.controller('vmTabCtrl', ['$scope', 'httpLoad', '$rootScope','$modal','$state','$timeout',
        function($scope, httpLoad, $rootScope, $modal,$state, $timeout) {
            $rootScope.moduleTitle = '资源中心 > 虚机管理';//定义当前页
            $rootScope.link = '/statics/css/user.css';

            var vmtabLocation = JSON.parse(localStorage.getItem('vmtabLocation'));
            $scope.active1 = $scope.active2 = $scope.active3 = $scope.active4 = $scope.active5 = $scope.active6= $scope.active7 = false;
            ...
            switch(vmTabvendor.type){
                ...
                case 'JDYUN':
                    ...
                    $scope.isShowsnapshot = $scope.isShowsize = $scope.isShownetwork = false;
                    break;
                case 'ALIYUN':
                    $scope.isShowvm = $scope.isShowaccount = $scope.isShowvolume = $scope.isShowsecurity = true;
                    $scope.isShowsnapshot = $scope.isShowsize = $scope.isShownetwork = false;
                ...
                case 'VMWARE':
                    $scope.isShowvm = true;
                    $scope.isShowaccount = $scope.isShowvolume = $scope.isShowsnapshot = $scope.isShowsize = $scope.isShownetwork = $scope.isShowsecurity = false;
                    break;
                default:
                    $scope.isShowvm = true;
                    $scope.isShowaccount = $scope.isShowvolume = $scope.isShowsnapshot = $scope.isShowsize = $scope.isShownetwork = $scope.isShowsecurity = false;
    ...
    app.controller('vmListCtrl', ['$scope', 'httpLoad', '$rootScope','$modal','$state','$timeout','$stateParams',
        function($scope, httpLoad, $rootScope, $modal,$state, $timeout,$stateParams) {
            ...
            var vmTabvendor = JSON.parse(localStorage.getItem('vmTabvendor'));
            if(vmTabvendor.type == 'OPENSTACK'||vmTabvendor.type == 'VMWARE'){
                $scope.vmTable = 1;
            }else if(vmTabvendor.type == 'JDYUN'||vmTabvendor.type == 'ALIYUN'){
                $scope.vmTable = 2;
            }
            $scope.getData(1);
            //状态数据
                    ...
                    success:function(data){
                        if(data.success&&data.data.rows&&data.data.rows.length!=0){
                            ...
                            angular.forEach($scope.networkList, function(data,index){
                                if(data.status=='ACTIVE') data.status1 = '运行中';
