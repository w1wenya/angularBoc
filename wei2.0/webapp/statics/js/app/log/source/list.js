...
(function(){
	...
	app.controller('SourceListCtrl', ['$scope', 'httpLoad', '$rootScope','$modal','$timeout','$state','LANGUAGE',
		function($scope, httpLoad, $rootScope, $modal, $timeout, $state, LANGUAGE) {
			$rootScope.moduleTitle = '日志中心 > 数据源管理';//定义当前页
			...
			$scope.goDetail = function(flag,id){
				switch (flag){
					case 1:
						$state.go('app.log.sourcedetail',{id:id});
						break;
					case 2:
						$state.go('app.log.sourceupload',{id:id});
						break;
					case 3:
						$state.go('app.log.sourcerecord',{id:id});
						break;
				}
			...
			$scope.create = function(id){
				$state.go('app.log.sourcecreate',{id:id})
			};
			//删除
