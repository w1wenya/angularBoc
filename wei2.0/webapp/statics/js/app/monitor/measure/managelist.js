...
(function(){
	...
	app.controller('MonitorManageListCtrl', ['$scope', 'httpLoad', '$rootScope','$modal','$timeout','$state','LANGUAGE',
		function($scope, httpLoad, $rootScope, $modal, $timeout, $state, LANGUAGE) {
			$rootScope.moduleTitle = '监控中心 > 监控项管理';//定义当前页
			$scope.param = {
				page: 1,
				rows: 10,
			}
			...
			$scope.goDetail = function(flag,id){
				switch (flag/1){
					case 1:
						$state.go('app.monitor.measureitem',{id:id});
						break;
					case 2:
						$state.go('app.monitor.measureplugin',{id:id});
						break;
					case 3:
						$state.go('app.monitor.measureupload',{id:id.id});
						sessionStorage.setItem('measureItem',id.name)
						break;
					case 4:
						$state.go('app.monitor.measurerecord',{id:id});
						break;
				}
			}
		}
