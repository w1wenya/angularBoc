        ...
        function($scope, httpLoad, $rootScope, $modal,$state, $timeout,LANGUAGE) {
            ...
            $scope.getData = function(page){
                ...
                if($scope.searchByTenantPhone&&$scope.searchByTenantPhone!=""){
                    searchParam.push({"param":{"tenantPhone":$scope.searchByTenantPhone},"sign":"LK"});
                }
                if($scope.searchByCompany&&$scope.searchByCompany!=""){
                    searchParam.push({"param":{"company":$scope.searchByCompany},"sign":"LK"});
                }
            ...
            $scope.detail = function(id){
                $state.go('app.userCenter.tenantDetail',{id:id});
            };
            //资源绑定
            $scope.bindHost = function(id){
                $state.go('app.userCenter.tenantResource',{id:id})
            };
            //新增
            ...
            $scope.user = function($event,id){
                $event.stopPropagation();
                $state.go('app.userCenter.tenantUser',{id:id});
            };
            //角色（展示租户下面的角色列表）
            $scope.role = function($event,id){
                $event.stopPropagation();
                $state.go('app.userCenter.tenantRole',{id:id});
            };
            //授权
